...
class Malheureux(Animal):
    behavior = {
        ...
        'nutrition': 50,
        'energy_gain': 30,
        'vision_radius': 3  # Радиус обзора
    }

# Класс мира с управлением временем через часы
class World:
    ...
    def restore_snapshot(self, hour: int):
        """Восстанавливает состояние мира из снимка"""
        ...
        for ent_data in snapshot['entities']:
            ...
            if class_name == 'Lumiere':
                entity = Lumiere()
            elif class_name == 'Obscurite':
                entity = Obscurite()
            elif class_name == 'Demi':
                entity = Demi()
    ...
    def update_time(self):
        ...
        self.time_of_day = self._calculate_time_of_day()
        # Сохраняем снимок при каждом изменении времени
        self.take_snapshot()
...
def draw_world(world: World, graph_elem, selected_animal: Optional[Animal] = None):
    ...
    # Отрисовка радиуса обзора выбранного животного
    if selected_animal:
        graph_elem.draw_circle(
...
def main():
    ...
    simulation_speed = 5  # шагов в секунду
    ...
    while True:
        ...
        # Обработка изменения времени через слайдер
        if event == '-HOUR-':
            new_hour = int(values['-HOUR-'])
            if new_hour != world.hour:
                # Пытаемся восстановить состояние для этого часа
                if world.restore_snapshot(new_hour):
                    ...
                    # Если снимка нет, возвращаем слайдер к текущему значению
                    hour_slider.update(world.hour)
        ...
        # Обработка наведения мыши на карте
        if event == '-MAP-':
        ...
        # Автоматический шаг при запущенной симуляции
